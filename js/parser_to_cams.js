...
let img_cam_arr = [], name_arr = [], rub_arr = [], cop_arr = [], info1_arr = [], info2_arr = [], info3_arr = [], info4_arr = [], way_arr = [];
let parser = (xmlString => {
  const parser = new DOMParser();
  const xmlDoc = parser.parseFromString(xmlString, 'text/xml');
  const img_cam = xmlDoc.getElementsByTagName('img_cam');
  ...
  img_cam_arr = table(img_cam, img_cam_arr);
  ...
  for (let i = 0; i < img_cam_arr.length; i++) {
    document.querySelector('.stuff').insertAdjacentHTML(
      "beforeend",
      `<div class="first">
              <div class="space"></div>
              <div class"img-container">
              <a href="${way_arr[i]}"><img src="${img_cam_arr[i]}" alt=""></a>
              </div>
              <div class="card">
